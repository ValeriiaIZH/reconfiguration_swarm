...
%     end
   ...
   disp('В графе коллизий имеются циклы! Алгоритм не работает.Применяем алгоритм "Разделяй и властвуй"');
    ...
    % уравнение прямой
    [S1, T1, S2, T2, numRob] = divide_and_conquer_V2(new_rob_cor, num_of_robots, ...
                                             TargetAll, num_of_robots); %КОЛИЧЕСТВО ЦЕЛЕЙ!!!!!!!!!!!!!!!!!!!!!!!!!!
    [PathKor_S1, PathTime_S1, ActiveDist_S1] = calculate_rob_to_tar(numRob, T1, S1);
...
%     end
    if isempty(Cycles)
    disp('В графе коллизий нет циклов.');
    add_delays(numRob,T1,S1,CollidedRobotsNum_S1);
    else
%         disp('NEXT');
        ActiveDist_1 = algorithm_2(Peak, Edge, numRob, S1, T1,...
                                 PathKor_S1,CollidedRobotsNum_S1);
    end
    end
    clear A E V;
    [PathKor_S2, PathTime_S2, ActiveDist_S2] = calculate_rob_to_tar(numRob, T2, S2);
    ActiveDist = [ActiveDist_S1; ActiveDist_S2];
    % проверка на коллизии
    [CollidedRobotsNum_S2,...
            ~] = my_collisiontest(PathKor_S1, PathTime_S2);
...
%     end
    if isempty(Cycles)
    disp('В графе коллизий нет циклов.');
    add_delays(numRob,T2,S2,CollidedRobotsNum_S2);
    else
%         disp('NEXT');
        ActiveDist_2 = algorithm_2(Peak, Edge, numRob, S2, T2,...
                                 PathKor_S2,CollidedRobotsNum_S2);
    end
    end
